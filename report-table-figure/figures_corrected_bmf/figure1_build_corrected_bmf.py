...
OUTPUT_PATH = ROOT / "report-table-figure" / "figures_corrected_bmf" / "figure1_used_in_report_corrected_bmf.png"
    ...
    ax.legend(
        handles=[
            Line2D(
                ...
                [0],
                color="#666666",
                linestyle=SPIN_LINESTYLES["no_spin"],
                marker=SPIN_MARKERS["no_spin"],
                markerfacecolor="white",
                markeredgecolor="#666666",
                markersize=6,
                linewidth=1.6,
                label=SPIN_LABELS["no_spin"],
            )
        ],
        loc="upper right",
        bbox_to_anchor=(1.0, 0.40),
...
def draw_panel_a(ax: plt.Axes, frame: pd.DataFrame) -> None:
    panel = frame.loc[
...
def draw_exact_archive_panel(ax: plt.Axes, subset: pd.DataFrame, title: str, y_max: float) -> None:
    subset = subset.sort_values(["v_inf_kms", "periapsis_Rm"])
    keep_velocities = sorted(v for v in subset["v_inf_kms"].dropna().unique() if float(v) <= 1.6)
